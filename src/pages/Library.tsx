...
import { MODE_LABELS, MODE_COLORS } from "../types/agent.ts"
import AgentCard from "../components/AgentCard.tsx"
import { useToast } from "../components/Toast.tsx"

type SortKey = "name" | "updatedAt" | "sessionCount" | "createdAt"
...
export default function Library() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterMode !== "all") {
      result = result.filter((a) => a.mode === filterMode)
    }
  ...
  return (
    <div>
          ...
          <p style={{ color: "#8a8aae", fontSize: "0.8rem", margin: "4px 0 0" }}>
            ...
            {agents.length > 0 && (
              <span style={{ marginLeft: 8 }}>
                · {agents.filter((a) => a.mode === "primary").length} primary · {agents.filter((a) => a.mode === "subagent").length} subagent
              </span>
            )}
          </p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <input ref={fileInputRef} type="file" accept=".md" multiple onChange={handleImport} style={{ display: "none" }} />
          <button className="btn-ghost" onClick={() => fileInputRef.current?.click()} style={{ fontSize: "0.8rem" }}>Import .md</button>
          <button className="btn-ghost" onClick={() => downloadAllAgents(agents)} style={{ fontSize: "0.8rem" }}>Export</button>
      ...
      <div style={{
        ...
        <select className="input-field" value={filterMode}
        ...
        <button onClick={toggleSortDir} className="btn-ghost"
      ...
      {hasActiveFilters && (
        <div style={{ display: "flex", gap: 4, marginBottom: "0.75rem", flexWrap: "wrap", alignItems: "center" }}>
          ...
          {filterMode !== "all" && (
            <span style={{ fontSize: "0.65rem", background: `${MODE_COLORS[filterMode as keyof typeof MODE_COLORS] || "#888"}20`, color: MODE_COLORS[filterMode as keyof typeof MODE_COLORS] || "#888", padding: "2px 8px", borderRadius: 12, display: "flex", alignItems: "center", gap: 4 }}>
              {MODE_LABELS[filterMode as keyof typeof MODE_LABELS] || filterMode}
